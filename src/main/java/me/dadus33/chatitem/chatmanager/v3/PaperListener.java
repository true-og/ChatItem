...
public class PaperListener implements Listener {
    ...
    public void onChat(AsyncChatEvent e) {
        ...
        if (ChatManager.containsSeparator(rawMessage)) { // fix for v1
            ...
            if (chat != null) {
                ...
                message = message.replaceText(
                        TextReplacementConfig.builder().matchLiteral(ChatManager.addSeparator(chat.getId()))
        ...
        if (action.hasItem()) {
            ...
            hoverEvent = HoverEvent.showText(C.text(
                    Messages.getMessage(action.getSlot().name().toLowerCase() + ".hover", "%cible%", p.getName())));
