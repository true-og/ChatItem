...
public abstract class ChatManager {
    ...
    public static String removeSeparator(String message) {
        ...
        return fixSeparator(message).replace(Character.toString(SEPARATOR), "")
                .replace(Character.toString(SEPARATOR_END), "");

    }

    public static String replaceSeparator(Chat chat, String message, String replacement) {

        return fixSeparator(message).replace(
                Character.toString(SEPARATOR) + (chat != null ? chat.getId() : "") + Character.toString(SEPARATOR_END),
                replacement);
    ...
    public static String fixSeparator(String s) {
        ...
        return s == null ? ""
                : s.replace(SEPARATOR_STR, Character.toString(SEPARATOR)).replace(SEPARATOR_END_STR,
                        Character.toString(SEPARATOR_END));
    ...
    public static boolean equalsSeparatorEnd(String s) {
        ...
        return s != null
                && (s.equalsIgnoreCase(SEPARATOR_END_STR) || s.equalsIgnoreCase(Character.toString(SEPARATOR_END)));
    ...
    public static ItemStack getUsableItem(Player p, ItemSlot slot) {
        ...
        if (EcoEnchantsSupport.hasSupport()) {
            ...
            } else if (meta instanceof BlockStateMeta && Version.getVersion().isNewerOrEquals(Version.V1_9)) { // if
                                                                                                               // it's a
                                                                                                               // block
    ...
    public static ChatAction getChatAction(ItemSlot slot, Player p) {
        ...
        if (slot.isCommand()) {
            ...
            InvShower.add(uuid.toString(),
                    slot == ItemSlot.INVENTORY ? new PlayerInventoryShower(p) : new EnderChestShower(p));
    ...
    public static String getNameForChatAction(Player viewer, Chat chat, Storage c) {
        ...
        return ChatItem.replace(chat.getPlayer(), Messages.getMessage(action.getSlot().name().toLowerCase() + ".chat",
                "%cible%", action.getOrigin().getName()));
    ...
    public static String getNameForChatAction(Player viewer, ChatAction action, Storage c) {
        ...
        return ChatItem.replace(action.getOrigin(), Messages.getMessage(action.getSlot().name().toLowerCase() + ".chat",
                "%cible%", action.getOrigin().getName()));
    ...
    public static String getHandName(Player p) {
        ...
        return ChatItem.replace(p, ChatItem.getInstance().getStorage().handName.replace("{name}", p.getName())
    ...
    public static boolean canUsePlaceholder(Player p, ChatAction action, ItemSlot slot, @Nullable Cancellable e) {
        ...
        LAST_INFO_MESSAGE.put(p.getUniqueId(), System.currentTimeMillis()); // prevent showing item then send cooldown
                                                                            // error message
        return true;
    ...
    public static boolean isSelected(String actual) {
        ...
        if (selected == "auto") {
            ...
            if (Utils.IS_PAPER && actual == "paper")
                return true;
            else if (actual == "packet" && ChatItem.getPluginThatRequirePacket().stream()
                    .map(Bukkit.getPluginManager()::getPlugin).anyMatch(Objects::nonNull))
                return true;
            else if (actual == "chat")
                return true;
