...
public class Translator {
        ...
        for (char args : old.toCharArray()) {
            ...
            if (args == '§') { // begin of color
                ...
                if (!colorCode.isEmpty()) { // manage color
                    ...
                    ChatItem.debug(
                            "[Translator] Add color code " + colorCode + " to " + text + args + ", msg: " + message);
                    colorCode = "";
    ...
    public static JsonArray toJsonOld(String old) {
        ...
        JsonObject next = null; // refers to the object we created before (in time) but next in the message, as
                                // we're going from end to start
        for (int i = parts.length - 1; i >= 0; --i) { // We go in reverse order
            ...
            if (part.length() == 1) { // If it's just format and no text, we try to format the next element, if it
                                      // wasn't formatted already (and it's not null)
            ...
            // Last possibility that remains is that we have a normal color/format + text
            // situation
            JsonObject added = new JsonObject();
        ...
        JsonArray orderedMessage = new JsonArray();// This is where we'll store the reverted message (in proper order
    ...
    private static String getStyleName(char code) {
        ...
        switch (code) {
            ...
            default:
                ...
                return null; // Should never happen. Made it return null to throw errors if a new format pops
                             // up and it really happens
