...
public class PacketEditingChatManager extends ChatManager {
    ...
    private static Object internalCreateSystemChatPacket(Object obj, Object old) throws Exception {
        ...
        Class<?> packetClass = PacketUtils.getNmsClass("ClientboundSystemChatPacket", "network.protocol.game.",
                "ClientboundPlayerChatPacket", "PacketPlayOutChat");
        Class<?> chatMessageTypeClass = Utils.isClassExist("net.minecraft.network.chat.ChatMessageType")
                ? PacketUtils.getNmsClass("ChatMessageType", "network.chat.")
                : null;
        ...
        for (Constructor<?> cons : packetClass.getDeclaredConstructors()) {
            ...
            if (obj instanceof String) {
                ...
                if (cons.getParameterCount() == 2 && cons.getParameterTypes()[0].equals(String.class)
                        && cons.getParameterTypes()[1].equals(int.class))
                { // "string, int"

                    return cons.newInstance(obj, 1);

                } else if (cons.getParameterCount() == 2 && cons.getParameterTypes()[0].equals(String.class)
                        && cons.getParameterTypes()[1].equals(boolean.class))
                { // "string, boolean"

                    return cons.newInstance(obj, false); // false for no overlay

                } else if (cons.getParameterCount() == 3 && cons.getParameterTypes()[1].equals(String.class)) { // "component",
                                                                                                                // "string",
                                                                                                                // <something
                                                                                                                // not
                                                                                                                // checked>
            ...
            } else if (obj.getClass().isAssignableFrom(PacketUtils.COMPONENT_CLASS)) {

                if (cons.getParameterCount() == 2
                        && cons.getParameterTypes()[0].isAssignableFrom(PacketUtils.COMPONENT_CLASS)
                        && cons.getParameterTypes()[1].equals(boolean.class)) // "string, boolean"
                    return cons.newInstance(obj, false);
            ...
            if (nbPut == 1) {
                ...
                ChatItem.getInstance().getLogger().warning("Some constructor seems to have too many "
                        + obj.getClass().getSimpleName() + ". Class: " + packetClass.getSimpleName());
        ...
        if (betterOne != null && betterParam != null) {
            ...
            try {
                ...
                ChatItem.getInstance().getLogger()
                        .warning("Failed to create a new packet to send. " + betterOne.getParameterCount() + " ("
                                + Utils.arrayToString(betterOne.getParameterTypes()) + "). Params: "
                                + Utils.arrayToString(betterParam));
                return null;
