...
public class ChatItemPacketManager {
        ...
        if (protocolLibPlugin != null) {
            ...
            if (checkProtocollibConditions()) {
                ...
                pl.getLogger().warning(
                        "The plugin ProtocolLib has been detected but you have an old or too new version, so we cannot use it.");
    ...
    private boolean checkProtocollibConditions() {
        ...
        for (String searchedClass : Arrays.asList("com.comphenix.protocol.injector.server.TemporaryPlayer",
                "com.comphenix.protocol.injector.temporary.TemporaryPlayer"))
        { // class since 4.4.0 until 4.8.0, then the new one
