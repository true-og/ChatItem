...
public class AdventureComponentManager implements IComponentManager {
    ...
    public void writeComponentToPacket(ChatItemPacket packet, Component next) {
        ...
        if (packet.getContent().getSpecificModifier(Component.class).readSafely(0) == null
                && ReflectionUtils.hasObject(packet.getPacket(), "unsignedContent"))
        { // hard way

            ReflectionUtils.setField(ReflectionUtils.getObject(packet.getPacket(), "unsignedContent"), "adventure",
                    next);
    ...
    public Component getComponentFromPacket(ChatItemPacket packet) {
        ...
        if (comp == null && packet.getPacketName().equalsIgnoreCase("ClientboundPlayerChatPacket")
                && ReflectionUtils.hasObject(packet.getPacket(), "unsignedContent"))
        { // if can get one more

            comp = (Component) ReflectionUtils
                    .getObject(ReflectionUtils.getObject(packet.getPacket(), "unsignedContent"), "adventure");
            ...
            throws Exception
    {
        ...
        if (action.hasItem()) {
            ...
            if (Utils.IS_PAPER)
                ...
                hover = HoverEvent.showItem(Key.key(item.getType().getKey().getKey()), item.getAmount(),
                        BinaryTagHolder.of(PacketUtils.getNbtTag(item)));
            return manage(viewer, chat, packet, itemName, hover, null);

        }

        return manage(viewer, chat, packet,
                Messages.getMessage(action.getSlot().name().toLowerCase() + ".chat", "%cible%",
                        chat.getPlayer().getName()),
                HoverEvent.showText(Component.text(Messages.getMessage(action.getSlot().name().toLowerCase() + ".hover",
    ...
    public Object manageEmpty(Player viewer, Chat chat, ChatItemPacket packet, String json, Storage c) {
        ...
        if (action.hasItem()) {
            ...
            return manage(viewer, chat, packet, ChatManager.getNameForChatAction(viewer, chat, c),
                    HoverEvent.showText(builder), null);
            ...
            ClickEvent click)
    {
        ...
        comp = comp.replaceText(TextReplacementConfig.builder()
                .matchLiteral(ChatManager.SEPARATOR + "" + chat.getId() + ChatManager.SEPARATOR_END)
                .replacement(Component.text(replacement).hoverEvent(hover).clickEvent(click)).build());
