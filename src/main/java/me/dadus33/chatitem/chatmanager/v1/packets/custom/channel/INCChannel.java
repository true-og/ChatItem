...
public class INCChannel extends ChannelAbstract {
    ...
    public List<ChannelFuture> getFuturChannel() {
        ...
        try {
            ...
            Object co = ReflectionUtils.getFirstWith(mcServer, PacketUtils.getNmsClass("MinecraftServer", "server."),
                    PacketUtils.getNmsClass("ServerConnection", "network."));
            try {
    ...
    public Channel getChannel(Player p) throws Exception {
        ...
        return new PacketContent(networkManager).getSpecificModifier(Channel.class).readSafely(0);// (Channel)
                                                                                                  // networkManager.getClass().getDeclaredField("channel").get(networkManager);
