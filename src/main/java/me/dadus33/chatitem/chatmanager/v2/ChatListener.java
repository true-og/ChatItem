    ...
    public void onChat(AsyncPlayerChatEvent e) {
        ...
        if (ChatManager.containsSeparator(e.getMessage())) { // fix for v1
            ...
            if (chat != null) {
                ...
                e.setMessage(
                        ChatManager.replaceSeparator(chat, e.getMessage(), chat.getSlot().getPlaceholders().get(0)));
        ...
        if (slot == null) { // if not found
            ...
            ChatItem.debug("(v2) Can't found placeholder in: " + e.getMessage() + " > "
                    + PlayerNamerManager.getPlayerNamer().getName(p));
            return;
        ...
        Set<Player> recipients = e.getRecipients().isEmpty() ? new HashSet<>(Bukkit.getOnlinePlayers())
                : e.getRecipients();
        ChatItem.debug("(v2) Msg: " + ChatItem.replace(p, msg).replace(ChatColor.COLOR_CHAR, '&') + ", format: "
                + format + " to " + recipients.size() + " players");
