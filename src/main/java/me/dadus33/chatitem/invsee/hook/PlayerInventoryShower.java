...
public class PlayerInventoryShower extends InvShower {
    ...
    public void open(Player p) {
        ...
        Inventory inv = ChatItem.getPlatform().createInventory(new CustomInventoryHolder(), 54,
                Messages.getMessage("inventory.name", "%cible%", name));

        for (int i = 0; i < 18; i++)
            inv.setItem(i, ItemUtils.ITEM_EMPTY_BROWN);

        inv.setItem(2, new ItemStack(ItemUtils.getMaterialWithCompatibility("EXPERIENCE_BOTTLE", "EXP_BOTTLE"),
                level == 0 ? 1 : (level >= 64 ? 64 : level)));
