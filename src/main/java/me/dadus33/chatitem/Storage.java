...
public class Storage {
    ...
    public String handName, nameFormat, amountFormat, commandFormat, manager, permissionName, buggedClientAction,
            language;
    ...
    public boolean colorIfColored, addAmountForced, letMessageThrough, debug, handDisabled, showNoPermissionMessage,
            checkUpdate, permissionEnabled, cmdBroadcast, cmdShow, discordSrvSendMsg, replaceInConsole;
    ...
    public Storage(FileConfiguration cnf) {
        ...
        tooltipHand = conf.getStringList("general.hand.tooltip").stream().map(Storage::color)
                .collect(Collectors.toList());
        buggedClientAction = conf.getString("general.bugged_client.action", "show_both");
        tooltipBuggedClient = conf.getStringList("general.bugged_client.tooltip").stream().map(Storage::color)
                .collect(Collectors.toList());
        ...
        messageLimit = color(
                conf.getString("messages.limit-message", "&c&lYou can only add 8 item placeholders per message!"));
