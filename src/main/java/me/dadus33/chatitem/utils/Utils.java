...
public class Utils {
    ...
    public static boolean isBeforeChatJson(Player p) {
        ...
        return !PlayerVersionManager.getPlayerVersionAdapter().getPlayerVersion(p).isNewerOrEquals(Version.V1_16)
                && Version.getVersion().isNewerOrEquals(Version.V1_16);
    ...
    public static HoverEvent createItemHover(ItemStack item, Player to) {
        ...
        if (Version.getVersion().isNewerOrEquals(Version.V1_13)) {
            ...
            return new HoverEvent(HoverEvent.Action.SHOW_ITEM, new Item(item.getType().getKey().getKey(),
                    item.getAmount(), ItemTag.ofNbt(PacketUtils.getNbtTag(item))));

        } else {

            return new HoverEvent(HoverEvent.Action.SHOW_ITEM,
                    new ComponentBuilder(ChatItem.getPlatform().stringifyItem(item)).create());
