...
public class Translation {
        ...
        allLangsConfig.getStringList("all").forEach(langKey -> {
            ...
            try {
                ...
                JsonObject content = JsonParser
                        .parseReader(new InputStreamReader(pl.getResource("lang/" + langKey + ".json")))
                        .getAsJsonObject();
                allLangs.put(langKey, content.get("language.name").getAsString() + " ("
                        + content.get("language.region").getAsString() + ")");

            } catch (Exception e) {

                pl.getLogger().severe("Failed to load lang with key " + langKey + " : " + e.getMessage() + " ("
                        + e.getStackTrace()[0].toString() + ")");

            }

        });
        allLangs = allLangs.entrySet().stream().sorted(Entry.comparingByValue())
    ...
    public static String get(ItemStack item) {
        ...
        String key = (item.getType().isBlock() ? "block" : "item") + ".minecraft."
        ...
        return legacy.getString(item.getType().name() + "." + item.getDurability(),
    ...
    protected void thisMethodShouldNotBeUsed_InsteadToUpdateLangFileFromLocalInstall() throws Exception {
        ...
        JsonObject json = JsonParser.parseReader(new FileReader(new File(folder, "indexes/1.19.json")))
