...
public class VaultPlaceholderHook implements IPlaceholders {
    ...
    public String replace(Player p, String text) {
        ...
        if (chat == null)
            return text;
        return text.replace("{prefix}", chat.getPlayerPrefix(p)).replace("{suffix}", chat.getPlayerSuffix(p))
                .replace("%prefix%", chat.getPlayerPrefix(p)).replace("%suffix%", chat.getPlayerSuffix(p));
