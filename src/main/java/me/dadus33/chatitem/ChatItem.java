...
import me.dadus33.chatitem.hook.placeholders.EssentialsPlaceholdersHook;
...
public class ChatItem extends JavaPlugin {
    ...
    private final String brandChannelName = Version.getVersion().isNewerOrEquals(Version.V1_13) ? "minecraft:brand"
            : "MC|Brand";
    ...
    public static void debug(String msg) {
        ...
        if (getInstance().getStorage().debug)
            getInstance().getLogger()
                    .info("[Debug] " + msg.replace(ChatManager.SEPARATOR, 'S').replace(ChatManager.SEPARATOR_END, 'E'));
    ...
    public void onEnable() {
        ...
        instance = this;
        getLogger().info("Detected server version: " + Version.getVersion().name().toLowerCase() + " (Platform: "
                + getPlatform().getName() + ")");
        ...
        if (pm.isPluginEnabled("ChatControl")
                && ReflectionUtils.isClassExist("org.mineacademy.chatcontrol.settings.Settings"))
        {
        ...
        if (pm.isPluginEnabled("Essentials")) {
            ...
            PLACEHOLDERS.add(new EssentialsPlaceholdersHook());
        ...
        getServer().getMessenger().registerIncomingPluginChannel(this, brandChannelName,
                (chan, p, msg) -> ItemPlayer.getPlayer(p).setClientName(new String(msg).substring(1)));
